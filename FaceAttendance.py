...
class App:
    ...
    def __init__(self, window, window_title, video_source=0):
        ...
        font = ('times', 16, 'bold')
        title = Label(window, text='Automatic Attendance Management System using Face Detection')
        ...
        self.btn_snapshot=tkinter.Button(window, text="Capture Face Image", command=self.capturePerson)
    ...
    def saveImage(self):
        ...
        if self.capture_img is not None:
            ...
            if self.isUserExist(class_label) == 0:
                ...
                f.write(str(class_label)+","+str(sid)+","+name+","+dt+"\n")
                f.close()
        ...
        if option == 1:
            ...
            for i in range(len(self.student_details)):
                ...
                if arr[0] == str(predict):
                    ...
                    self.text.insert(END,str(datetime.now())+"\n\n")
                    flag = 1
                    break
            if flag == 0:
